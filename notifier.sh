...
if [ -z $INITIAL ]; then
        ...
        if [ -z $BLOCK ]; then
                ...
                if [ "$INITIAL" -eq "$BLOCK" ]; then
                        echo "Node is stop"
                        curl -s -X POST https://api.telegram.org/bot$BOTNUMBER/sendMessage -d chat_id=$CHATID -d text="☠️ Node $NAME $IPN stopped at block $BLEOS. Please check http://$IPN/api/v1/status/peer"
                fi
        fi
fi
}
